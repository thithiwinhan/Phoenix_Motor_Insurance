		...
		if(updateDB.isPresent()) {
			...
			System.out.println("********************************Update Proposal Dao*********************");
		      ...
		      userDao.save(user);
			   ...
			   System.out.println("-------------Vehicle-----------"+updateProposal.getVehicle().getId() +"db<<<<>>>>>>ui"+proposal.getVehicleID());
			   ...
			   // vehicle.setCurrentValue(proposal.getCurrentValue());
		        ...
		        System.out.println("-------------PolicyHolder-----------"+updateProposal.getPolicyHolder().getId()+"db>>>>>>>>>.ui"+proposal.getpHolderID());
		        PolicyHolder policyHolder = new PolicyHolder();
		        ...
		        System.out.println("-------------proposal -----------"+updateProposal.getPolicyHolder().getId()+"db>>>>>>>>>.ui"+proposal.getpHolderID());
				Proposal p = new Proposal();
...
//check if status pending  according to given pid , if pending is found in proposal table  ,return true
	...
	public Proposal findProposalById(int proposalId) {
		 ...
		 else System.out.println(" Resource is not found");
		return p;
	}
